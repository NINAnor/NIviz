...
```
Before proceeding any further, we need to force all indicator values > 1 to display as 1. 

```{r, eval = TRUE}

bootStrp$scaledIndicator[which(bootStrp$scaledIndicator > 1)] <- 1
...
for(t in years){
  ...
  sum_values <- bootStrp_yr %>% 
    group_by(ICunitName) %>%
    summarise(sumStat = median(scaledIndicator)) 
  
  # Plot densities
  print(
    ggplot(pDens_yr, aes(x, y)) + 
