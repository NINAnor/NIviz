...
for(t in years){
  ...
  if(maxVal < 1+1/9){
    valuesMap <- seq(0, 1, length.out = 10)
    colorMap <- NIviz_colours$IndMap_cols
  }else{
    #valuesMap <- c(seq(0, 1, length.out = 10), seq(1+1/9, maxVal, by = 1/9))/maxVal
    #colorMap <- c(NIviz_colours$IndMap_cols, rep("#4B4BAF", length(valuesMap)-10))
    valuesMap <- c(seq(0, 1, length.out = 10), maxVal)/maxVal
    colorMap <- c(NIviz_colours$IndMap_cols, "#4B4BAF")
  }
  ...
  print(
    ggplot(subset(pDens_yr, x <= maxVal), aes(x, y)) + 
      ...
      scale_colour_gradientn(colours = colorMap,
                             values = valuesMap,
                             limits = c(0, ifelse(maxVal < 1, 1, maxVal))) +
      ggtitle(paste0(i, " (", t, ")")) +
      xlab("Value") + 
