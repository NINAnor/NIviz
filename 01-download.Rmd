...
#path <- "P:/41201612_naturindeks_2021_2023_database_og_innsyn#slosning/temp"
#
#for(i in indicator){
#  temp <- get(paste0(i, "_import"))
#  saveRDS(temp, paste0(path, ".rds"))
#}
```
...
myMat2 <- myMat2 %>%
  tibble::add_column(.before=1,
    NIunitID = row.names(myMat))
myMat2 <- myMat2 %>%
  tibble::add_column(.after = 1,
    NIunitName = temp2$ICunits$name[match(
      myMat2$NIunitID, temp2$ICunits$id)],
    year = myYears[i]) 
